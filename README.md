...
Record a short screencast covering:
- User registration and login in the frontend (`/signup`, `/login`).
- Calling `/generate-report` with a `session_id` (via the frontend or cURL/Postman).
- Showing the generated PDF appearing under `Backend/reports/`.
- Opening the generated PDF in the browser.
- Bonus: Edit `Backend/config.json` (e.g., add a field or tweak classification) and regenerate to show how easily the report changes.
...
3. Sign up or log in.
4. Enter a sample Session ID (e.g., `session_001`, `session_002`).
5. Click “Open PDF” to view the generated report.

---

## Troubleshooting
- Missing values: If a field shows `Not available`, verify the path in `Backend/config.json` matches the data in `Backend/data.js` (or your source).
- Puppeteer launch issues: Ensure Node version compatibility and OS supports headless Chromium.
- CORS: Adjust `cors()` settings in `Backend/index.js` if needed.
- Auth: Include the `Authorization: Bearer <token>` header for `/generate-report`.

---

## Tech Stack
- Backend: Node.js, Express, Handlebars, Puppeteer, JWT, Helmet, express-rate-limit
- Frontend: React (CRA), Tailwind (available), custom CSS
